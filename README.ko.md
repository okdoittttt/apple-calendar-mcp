...
없습니다. Claude Desktop과 함께 사용하도록 만들어졌지만, 모든 MCP stdio
클라이언트와 함께 작동합니다.

기본 도구 세트는 [harriscarl/apple-eventkit-mcp](https://github.com/harriscarl/apple-eventkit-mcp)와
일치합니다. 이 포크는 태그 관리(`tags_*`), 일정 관리 편의 도구
(`agenda_*`, `quick_event`, `reschedule_event`, `find_free_slots`), 그리고
핫 리스타트 관리 도구(`server_status`, `server_restart`)를 추가합니다.
  ...
  "mcpServers": {
    "apple-eventkit-mcp": {
      ...
      "args": [
        ...
        "run", "python", "-m", "apple_eventkit_mcp.server"
      ]
    }
  }
}
...
| `calendar_edit_event` | 이벤트 편집; 반복 이벤트는 `span` 필수 |
...
호출하여 현재 프로세스 이미지를 새로운 `python -m apple_eventkit_mcp.server`로
...
1. 도구를 한두 번 다시 호출해 보세요 - 일부 MCP 클라이언트는 오류 시
   투명하게 재시도/재초기화합니다.
2. 그래도 도움이 되지 않으면, Claude Desktop 설정에서 `apple-eventkit-mcp`
...
```bash
uv run python -m apple_eventkit_mcp.server   # 독립 실행 (stdio 읽기/쓰기)
...
```bash
npx @modelcontextprotocol/inspector uv run python -m apple_eventkit_mcp.server
```

Node를 사용할 수 없었기 때문에, 대신 `mcp` Python SDK 자체 클라이언트
(`ClientSession` + `stdio_client`)로 프로토콜 계층을 검증했습니다: 전체
`initialize` 핸드셰이크, `list_tools` (27개 도구), 그리고 대표적인 도구
